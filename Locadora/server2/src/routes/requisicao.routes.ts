...
requisicaoRouter.route("/")   //get para a rota inicial
    ...
    requisicaoRouter.route("/devolver")  //devolver req 
    .post((req: Request, res: Response) => {
        var id:number= req.body.Id;
        
        let devolvido = requisicaoController.devolveReq(id); 
        
        if (devolvido) {
            return res.json({Sucesso:"Devolução realizada com sucesso"})
        }else{
            return res.status(409).json({Falha:"Ops, devolução não aprovada"})
        }
